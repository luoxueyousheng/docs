...
<h3 id="url-scheme">自定义 URL 协议（`register_url_scheme` / `unregister_url_scheme`）</h3>
...
<h3 id="file-association">文件类型关联（`register_file_association` / `unregister_file_association`）</h3>
...
<h3 id="global-hotkey-api">全局热键（`register_global_hotkey` / `unregister_global_hotkey`）</h3>
