...
| `default_menu_names` *(2.2 新增)* | WebView2 默认菜单项列表，每项含 `name`、`label`、`kind`（见 [菜单项类型与原生菜单项](/v2api/context-menu-items)） |
...
菜单项类型说明、`kind=5` 默认菜单项的 WebView2 原生 Name 列表（按右键上下文分类）详见 [菜单项类型与原生菜单项](/v2api/context-menu-items)。
