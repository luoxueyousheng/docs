...
页面通过 `postMessage` 发送消息。**仅当来源通过白名单**时才会触发（见 [核心 API · WebViewSettings](/v2api/index#webview-settings)）。
...
右键菜单相关事件（`context-menu`、`menu-item-clicked`）已移至 [右键菜单](/v2api/context-menu-api) 独立文档。
