...
| kind | 说明 | label 含义 | 点击行为 |
|------|------|-----------|---------|
| 0 | 命令 | 显示文本 | 发 `menu-item-clicked` 事件 |
| 1 | **复选框** | 显示文本 | 发 `menu-item-clicked` 事件 |
| 2 | **单选** | 显示文本 | 发 `menu-item-clicked` 事件 |
| 3 | **分隔线** | 忽略 | 无 |
| 4 | 子菜单 | 显示文本 | 展开子菜单 |
| **5** | **默认菜单项** | **WebView2 Name（如 `"copy"`）** | **WebView2 原生操作，不发事件** |

> **⚠️ 注意**：kind 枚举值 1/2/3 在早期版本中映射有误（1=分隔线、2=复选框、3=单选），现已修正为上述对应关系。此修正影响 `context-menu` 事件的 `default_menu_names` 字段中的 `kind` 值。
...
:::warning
Name 列表取决于 WebView2 Runtime 版本和右键上下文，部分项仅在特定场景下可用（如 `copy` 仅在选中文本时存在）。如果指定的 Name 在当前右键上下文中不存在，`jade_menu_item_create` 将返回 `0`。[context-menu](/v2api/context-menu-api) 事件的 `default_menu_names` 字段会返回当前右键上下文中实际可用的完整列表。
:::
