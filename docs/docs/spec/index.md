...
5. **注册其他事件回调**：通过 `jade_on` 注册其他事件回调
6. **清理资源**：监听 `window-all-closed` 事件，调用 `jadeview_exit` 清理资源
...
4. **回调函数返回值**：窗口事件回调函数返回 `const char*`：NULL=允许/默认，非空字符串=阻止/自定义应答
5. **资源管理**：使用完毕后应及时调用 `jadeview_exit` 清理资源
