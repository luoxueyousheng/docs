...
JadeView 通过 `set_protocol_service_path` 注册自定义协议处理器，将 `JADE://{app_signature}`（域名取自 `app_signature` 的小写形式）映射到本地目录或 JAPK 文件，实现安全、高效的本地资源加载。

:::warning{title="重要提示"}
`JADE://{app_signature}` 是 JadeView 内部的自定义协议路由，不是真实的 HTTP 服务器。它不会监听端口，完全在应用内部运行。
:::

## 前提条件

- 从 [GitHub Releases](https://github.com/JadeViewDocs/JadeView/releases) 下载 `JadeView_x64.dll` 和 `JadeView.h`
- 了解 C 语言基础
...
const char* app_ready_callback(uint32_t window_id, const char* event_data) {
    if (window_id == 1) {  // window_id==1 即成功；event_data 为 JSON，失败时（window_id==0）为纯文本错误码
        ...
        // 设置本地协议服务 —— 将 ./web 目录映射到 JADE://com.example.myapp
        char url_buffer[256];
        int result = set_protocol_service_path(
            "./web",              // 本地资源目录
            url_buffer,           // 输出协议 URL 缓冲区
            sizeof(url_buffer),   // 缓冲区大小
            0                     // hot_reload: 0=禁用, 1=启用
        );

        if (result == 1) {
            printf("协议 URL: %s\n", url_buffer);
            // url_buffer 内容类似: JADE://com.example.myapp
...
int main() {
    ...
    int result = JadeView_init(
        ...
        NULL,                   // data_directory
        "我的应用",              // app_name (应用显示名)
        "com.example.myapp",    // app_signature (小写后用作协议域名)
        0                       // single_instance
    );
...
1. `set_protocol_service_path("./web", url_buffer, size, 0)` 将本地目录注册到协议路由
2. 返回的 URL 格式为 `JADE://{app_signature}`（`app_signature` 小写）
3. 当 WebView 访问 `JADE://com.example.myapp/index.html` 时，JadeView 直接从 `./web/index.html` 读取
...
> 更多信息：[JAPK 资源包](/docs/api/japk) | [从内存载入](/docs/api/japk-load-memory)

---

## 注意事项

1. **不是 HTTP 服务器**：`JADE://{app_signature}` 不监听端口，外部浏览器无法访问
2. **`jade.invoke` 需要 CORS 白名单**（v2.1+）：如果页面不是通过协议服务加载的，需在 `WebViewSettings.cors_whitelist` 中设置允许的来源
3. **`app_signature` 决定域名**：协议 URL 的域名取自 `JadeView_init` 的 `app_signature` 参数（小写），而非 `app_name`
4. **页面路径映射**：`JADE://com.example.myapp/css/style.css` → `./web/css/style.css`

> 详细 API：[核心 API - 本地协议服务](/docs/api#本地协议服务) | [前端通信 API](/docs/api/javascript-api)