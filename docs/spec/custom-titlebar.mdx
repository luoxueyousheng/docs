...
1. **`title-overlay`**：使用内置标题栏按钮覆盖层（Windows 专属）
...
`title-overlay` 是 Windows 专属的边框样式，提供系统级标题栏按钮（最小化/最大化/关闭），无需自己实现。
...
<body>
  ...
  <script>
    // 窗口控制 - 通过 JavaScript API 或 IPC 调用宿主
    document.getElementById('minBtn').onclick = () => {
      window.__jadeview?.minimizeWindow?.();
    };

    document.getElementById('maxBtn').onclick = () => {
      window.__jadeview?.toggleMaximize?.();
    };

    document.getElementById('closeBtn').onclick = () => {
      window.__jadeview?.closeWindow?.();
    };
  </script>
</body>
</html>
