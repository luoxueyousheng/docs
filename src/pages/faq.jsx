...
export default function FAQPage() {
  ...
  function Pill({ tag }) {
    const tagText = tag === 'All' ? '所有' : tag === 'JadeView' ? 'JadeView' : tag === 'General' ? '通用' : tag === 'Platform' ? '平台' : tag === 'Features' ? '功能' : tag === 'Getting Started' ? '快速开始' : tag === 'Memory Management' ? '内存管理' : tag === 'Window Management' ? '窗口管理' : tag === 'Events' ? '事件' : tag === 'Theme' ? '主题' : tag === 'IPC' ? 'IPC通信' : tag === 'Local Server' ? '本地服务器' : tag === 'Debugging' ? '调试' : tag === 'Troubleshooting' ? '故障排除' : tag === 'Best Practices' ? '最佳实践' : tag;
  ...
  return (
    <Layout
      title="常见问题解答"
      description="找到关于 JadeView 的常见问题解答。探索解决方案并深入了解相关查询。"
        ...
        <div className="my-8 text-center text-4xl font-bold leading-tight text-zinc-800 dark:text-zinc-100 lg:text-6xl">
          ...
          <div className="relative flex w-full max-w-md items-center text-zinc-700 dark:text-white">
            ...
            <input
              ...
              placeholder="搜索你的问题...(sdk, api, 编写代码)"
              value={query}
        ...
        <div className="mx-auto max-w-7xl">
          {query.trim() !== '' ? (
            filteredFAQs.length === 0 ? (
              ...
              🙌 显示 {filteredFAQs.length} 个结果，搜索关键词：&quot;{query}&quot;
            </div>
            )
          ) : (
